...
function HaTTY(elem, opts) {
  ...
  this.elem._hatty = this;
  
  this._screenCoordToTerminal = function(sx, sy) {
    return {
      'x': Math.floor(sx / this.charsize.w) + 1,
      'y': Math.floor(sy / this.charsize.h) + 1,
    }
  }

  this._propagateEvent = function(name, ev) {
    for(var id in this._event_listeners[name]) {
      var handler = this._event_listeners[name][id];
      handler(ev);
    }
  };

  this.elem.addEventListener('click', function(e) {
    this._hatty._propagateEvent('click', {
      'originalEvent': e,
      'pos': this._hatty._screenCoordToTerminal(e.offsetX, e.offsetY)
    });
  });

  this.elem.addEventListener('mousemove', function(e) {
    this._hatty._propagateEvent('move', {
      'originalEvent': e,
      'pos': this._hatty._screenCoordToTerminal(e.offsetX, e.offsetY)
    });
  });
    ...
    if(c == '\x1b' && !force) {
      ...
      if(c == '[') {
        ...
        if(c == 'm') {
          ...
          this._resetCSI();
        } else if('ABCD'.indexOf(c) >= 0) {
          var movs = parseInt(this.buf || '1');
          // ABCD => up down right left
          var moveLUT = {
            'A': [0, -1],
            'B': [0, 1],
            'C': [0, 1],
          ...
          if(!isNaN(movs) && movs > 0) {
            var offX = moveLUT[c][0] * movs,
                offY = moveLUT[c][1] * movs;
            this.cursor.x = Math.min(this.cursor.x + offX, this.size.w - 1);
            this.cursor.y = Math.min(this.cursor.y + offY, this.size.h - 1);
          }
          this._resetCSI();
          return this;
        } else if(c == 'K') {
          ...
          if(this.buf == '' || this.buf == '0')
            ...
            this._clearRegion(0, this.cursor.y, this.cursor.x, this.cursor.y);
          else if(this.buf == '2')
            this._clearRegion(0, this.cursor.y, this.size.w - 1, this.cursor.y);
          this._resetCSI();
        } else if(c == 'H' || c == 'f') {
          var pos_arr = this.buf.split(';');
          var pos = {
            y: isNaN(pos_arr[0]) ? 0 : parseInt(pos_arr[0]),
            x: isNaN(pos_arr[1]) ? 0 : parseInt(pos_arr[1]),
          }
          this.cursor.x = Math.max(0, Math.min(pos.x, this.size.w - 1));
          this.cursor.y = Math.max(0, Math.min(pos.y, this.size.h - 1));
          this._resetCSI();
        } else if(c == 'h' && this.buf == '?9') {
          this._internals.interlaced = true;
          this._resetCSI();
        } else if(c == 'l' && this.buf == '?9') {
          this._internals.interlaced = false;
          this._resetCSI();
        } else {
          this.buf += c;
          return this;
  ...
  this._resetCSI = function() {
    this.buf = '';
    this._csi = this._esc = false;
  }
  
  this._clearRegion = function(sx, sy, ex, ey) {
    var ssx = sx * this.charsize.w,
        ssy = sy * this.charsize.h,
        sw = this.charsize.w * (ex - sx + 1),
        sh = this.charsize.h * (ey - sy + 1);
    this.ctx.beginPath();
    this.ctx.clearRect(ssx, ssy, sw, sh);
    this.ctx.closePath();
  }
  ...
  this._insert = function(y, x, fg, bg, c) {
    ...
    this.ctx.fill();
    this.ctx.closePath();
    if(this._internals.interlaced) {
      var cw = this.charsize.w, ch = this.charsize.h;
      this.ctx.beginPath();
      for(var oy = 0; oy < ch; oy += 2)
        this.ctx.clearRect(x * cw, y * ch + oy, cw, 1);
      this.ctx.closePath();
    }
  }
  ...
  this._randomString = function() {
    var out = '';
    for(var i = 0; i < 4; i++)
      out += Math.random().toString(16).substr(2, 8)
    return out;
  }
  
  this.addEventListener = function(name, callback) {
    if(!(name in this._event_listeners))
      throw new Error('invalid event type');
    var id = this._randomString();
    this._event_listeners[name][id] = callback;
    return id;
  }
  
  this._renderScanlines = function() {
    for(var y = 0; y < this.elem.height; y += 2)
      this.ctx.clearRect(0, y, this.elem.width, 1);
  }
